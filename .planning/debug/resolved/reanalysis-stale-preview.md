...
- timestamp: 2026-06-17
  ...
  verification: "`npx tsc --noEmit` and `npx vitest run` (176/176) both clean. User confirmed in-browser (2026-06-18): logo selection now persists and shows correctly in the Step 6 preview."

## Resolved: accent color had no preview consumer (feature gap — implemented this session)
...
- timestamp: 2026-06-17
  ...
  resolution: User specified the intended placement: DiagonaalKortti's sport pill background + "show on map" button background (with contrast-aware icon color), and a new rotating conic-gradient ring around CalloutCard's outer edge (same visual idea as the map pin's `.pin-arc` in globals.css, recolored to the accent). Implemented across `DiagonaalKortti.tsx`, `CalloutCard.tsx`, `StepEsikatselu.tsx` (sourcing `accentColor` the same way as `brandColor`), and `globals.css` (added `@property --ring-angle` + `ringAngleSpin` keyframe so the ring's gradient angle animates without rotating the card's non-circular silhouette — rotating the whole element, as in a first attempt, visibly spun the notched shape into a moving diamond).
  follow-up fixes during verification: (a) the ring's "light" gradient stop initially used an alpha-transparent accentColor, which washed out to near-invisible against the white page background mid-rotation, looking like a gap — fixed with an opaque `lightenHex()` helper instead. (b) `.glass`'s own 1px border AND its top-edge inset box-shadow sheen (both normally invisible against the white page) became a visible seam between the card and the new ring — removed both when `accentColor` is present. (c) widened the ring's margin (`RING_WIDTH` 5→8) for tolerance against sub-pixel rounding.
  verification: `npx tsc --noEmit` and `npx vitest run` (176/176) clean throughout. User confirmed in-browser (2026-06-18): ring sits flush all around with no gaps, button/pill colors correct, full flow re-checked and confirmed working.
