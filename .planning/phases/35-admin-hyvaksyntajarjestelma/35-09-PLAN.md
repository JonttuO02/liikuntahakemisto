...
must_haves:
  truths:
    - "app/admin/[id]/page.tsx is a Server Component with same auth + is_admin guards as admin page"
    - "Page displays company info, venue info, photos, pricing, contact, and application status"
    - "Non-admin gets 404, unauthenticated gets redirect('/')"
    - "Invalid (non-numeric) ID gets 404"
    - "Photo URLs are constructed from SUPABASE_URL + storage path"
    - "File compiles without TypeScript errors"
  artifacts:
    - path: "app/admin/[id]/page.tsx"
      provides: "Admin detail page for reviewing full application data including photos and all venue fields"
  key_links:
...
<objective>
Create the detail page that admin opens via "Tarkastele →" to inspect all venue data, media, and onboarding-provided info before approving or rejecting.

Purpose: ADMIN-02 requires that admin can see the full application details — not just the list summary. This page shows company info, applicant role, email, venue details, all uploaded photos, pricing, and contact info.

Output:
- app/admin/[id]/page.tsx — Server Component detail page
</objective>
...
<context>
Check if app/admin/[id]/ directory exists. Create it if not.

## Task 1: Create app/admin/[id]/page.tsx — Server Component

Note on photo_urls: The `photo_urls` column stores Supabase Storage paths (not full URLs). Construct the public URL with this helper at the top of the file:
```typescript
...
export default async function AdminDetailPage({ params }: { params: { id: string } }) {
  ...
  const photoUrls: string[] = Array.isArray(paikka?.photo_urls) ? (paikka.photo_urls as string[]).map(storageUrl) : []
...
Note: Adjust import paths (createServerSupabase, supabaseAdmin) to match what the project uses — check app/admin/page.tsx from plan 35-08 first.

## Verification

- Navigate to /admin/[id] for an existing pending application → see all venue details
- Back link ← returns to /admin list
- Non-admin gets 404
- Invalid ID (non-numeric) gets 404
- Photos display with storageUrl() wrapping
</context>
