...
- **D-14 (resolved via /gsd-plan-phase research gray-area):** `business_paikka_links` RLS SELECT stays **self-scoped** (`business_account_id` resolves to the same identity as before, just expressed via `current_company_id()` internally instead of a literal `auth.uid()` check) — NOT widened to company-wide visibility. No observable behavior change for existing (single-member) companies. Company-wide visibility widening is explicitly deferred to Phase 60/64, alongside the invite/role-management UI that would make a second company member exist in the first place. RESEARCH.md's Open Question 1 / Assumption A4 flagged this as the phase's single highest-risk ambiguity; this decision resolves it.
