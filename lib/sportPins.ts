...
const g = (content: string) =>
  `<g stroke="white" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round">${content}</g>`
...
export function pinUrl(color: string, laji: string): string {
  ...
  const iconSvg = iconContent
    ? `<svg x="9" y="9" width="10" height="10" viewBox="0 0 24 24">${iconContent}</svg>`
    : `<circle cx="14" cy="14" r="5" fill="white" opacity="0.85"/>`

  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 38" width="28" height="38"><path d="M14 0C6.268 0 0 6.268 0 14c0 5.25 2.875 9.83 7.125 12.3L14 38l6.875-11.7C25.125 23.83 28 19.25 28 14 28 6.268 21.732 0 14 0Z" fill="${color}"/>${iconSvg}</svg>`
